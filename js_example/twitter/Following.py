...
class Following:
    def __init__(self,user_id: int = 44196397,user_name : str = "elonmusk") -> None:
        ...
        self.variables = {"userId":str(user_id),"count":20,"includePromotedContent":False,"withDownvotePerspective":False,"withReactionsMetadata":False,"withReactionsPerspective":False}
        ...
        self.refreshCookie()
        self.query_id = getApiQueryId("Following",self.session)
    ...
    def crawl(self):
        ...
        self.url = f'https://twitter.com/i/api/graphql/{self.query_id}/Following"?variables={json.dumps(self.variables).replace("True","true").replace("False","false")}&features={json.dumps(self.features).replace("True","true").replace("False","false")}'
