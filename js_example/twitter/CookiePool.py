...
from Utils import getGuestToken
import urllib
...
class CookiePool:
    ...
    def _subTask(self):
        ...
        self.base_headers.update(
            {"path": "/1.1/onboarding/task.json", "cookie": self.cookie}
        )
        payload = {
            "flow_token": self.flow_token + "0",
            "subtask_inputs": [
                {
                    "subtask_id": "LoginJsInstrumentationSubtask",
                    "js_instrumentation": {
                        "response": json.dumps(self.rf),
                        "link": "next_link",
                    },
                }
            ],
        ...
        self.logger.info(payload["flow_token"])
                        ...
                        "setting_responses": [
                            {
                                "key": "user_identifier",
                                "response_data": {
                                    "text_data": {"result": self.login_email}
                                },
                            }
                        ],
                        "link": "next_link",
            ...
            "subtask_inputs": [
                {
                    ...
                    "enter_text": {"text": self.user_name, "link": "next_link"},
                }
            ],
        }
        res = self.session.post(
    ...
    def _subEmailCode(self) -> None:
        time.sleep(30)
        with Email(
            email_address=self.login_email,
            password=self.imap_email_password,
            ...
            "subtask_inputs": [
                {
                    ...
                    "enter_text": {"text": self.email_code, "link": "next_link"},
                }
            ],
        }
        res = self.session.post(
    ...
    def _subAfterLogin(self) -> None:
        query_id = "okNaf-6AQWu2DD2H_MAoVw"
        variables = {
        ...
        variables = urllib.parse.quote(
            f'{json.dumps(variables).replace("True","true").replace("False","false").replace(" ","")}'
        )
        features = urllib.parse.quote(
            f'{json.dumps(features).replace("True","true").replace("False","false").replace(" ","")}'
        )
        url = f"https://twitter.com/i/api/graphql/{query_id}/Viewer?variables={variables}&features={features}"
        self.base_headers.update({"path": url[19:], "cookie": self.cookie})
