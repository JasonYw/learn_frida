...
class Tweets:
    def __init__(self, user_id: int = 44196397, user_name: str = "elonmusk") -> None:
        ...
        self.variables = {"userId": str(self.user_id), "count": 40, "includePromotedContent": True, "withQuickPromoteEligibilityTweetFields": True,
    ...
    def crawl(self) -> None:
        ...
        data = res.json()[
            "data"]["user"]["result"]["timeline_v2"]["timeline"]["instructions"][-2]["entries"]
        if len(data) < 40:
            self.next_cursor = None
        else:
