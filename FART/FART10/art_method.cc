...
namespace art
{
  ...
  // add
  extern "C" void saveDexByExecute(ArtMethod *artMethod) REQUIRES_SHARED(Locks::mutator_lock_)
  {
    char *dexFilePath = (char *)malloc(sizeof(char) * 1000);
    if (dexFilePath == nullptr)
    {
      LOG(ERROR) << "ArtMethod::saveDexByExecute, methodName:" << artMethod->PrettyMethod().c_str() << "malloc 1000 byte failed";
      return;
    }
    else
    {
      LOG(ERROR) << "ArtMethod::saveDexByExecute, malloc 1000 byte success";
    }
    int result = 0;
    int fcmdline = -1;
    char szCmdline[64] = {0};
    ...
    if (fcmdline > 0)
    {
      LOG(ERROR) << "ArtMethod::saveDexByExecute, open cmdline file success";
      result = read(fcmdline, szProcName, 256);
      if (result < 0)
      {
        LOG(ERROR) << "ArtMethod::saveDexByExecute, read cmdline file error";
      }
      else
      {
        LOG(ERROR) << "ArtMethod::saveDexByExecute, read cmdline file success";
      }
      close(fcmdline);
    }
    else
    {
      LOG(ERROR) << "ArtMethod::saveDexByExecute, open cmdline file error";
    }
    ...
    if (szProcName[0])
    {
      ...
      {
        LOG(ERROR) << "ArtMethod::saveDexByExecute, dex is exist";
      }
      else
      {
        ...
        if (fp > 0)
        {
          LOG(ERROR) << "ArtMethod::saveDexByExecute, open dexfile success";
          result = write(fp, (void *)begin_, size_);
          if (result < 0)
          {
            LOG(ERROR) << "ArtMethod::saveDexByExecute, write dexfile error";
          }
          fsync(fp);
          close(fp);
          memset(dexFilePath, 0, 1000);
          ...
          if (classlistFile > 0)
          {
            LOG(ERROR) << "ArtMethod::saveDexByExecute, open classlistFile success";
            for (size_t ii = 0; ii < dex_file->NumClassDefs(); ++ii)
            {
              ...
              if (result < 0)
              {
                LOG(ERROR) << "ArtMethod::saveDexByExecute, write classlistFile file error1";
              }
              else
              {
                LOG(ERROR) << "ArtMethod::saveDexByExecute, write classlistFile file success1";
              }
              const char *temp = "\n";
              result = write(classlistFile, (void *)temp, 1);
              if (result < 0)
              {
                LOG(ERROR) << "ArtMethod::saveDexByExecute, write classlistFile file error2";
              }
              else
              {
                LOG(ERROR) << "ArtMethod::saveDexByExecute, write classlistFile file success2";
              }
            }
            fsync(classlistFile);
            close(classlistFile);
          ...
          {
            LOG(ERROR) << "ArtMethod::saveDexByExecute, open classlistFile error";
          }
        }
        else
        {
          LOG(ERROR) << "ArtMethod::saveDexByExecute, open dexfile error";
        }
      }
    }
  ...
  extern "C" void saveArtMethod(ArtMethod *artMethod) REQUIRES_SHARED(Locks::mutator_lock_)
  {
    ...
    if (szProcName[0])
    {
      ...
      memset(dexFilePath, 0, 1000);
      sprintf(dexFilePath, "/data/data/%s/xiaojianbang/%d_dexfile_artMethod.dex", szProcName, size_int_);
      ...
      else
      {
        ...
        if (fp > 0)
        {
          ...
          close(fp);
          memset(dexFilePath, 0, 1000);
          sprintf(dexFilePath, "/data/data/%s/xiaojianbang/%d_classlist_artMethod.txt", szProcName, size_int_);
          int classlistFile = open(dexFilePath, O_CREAT | O_APPEND | O_RDWR, 0666);
          if (classlistFile > 0)
          {
      ...
      {
        memset(dexFilePath, 0, 1000);
        sprintf(dexFilePath, "/data/data/%s/xiaojianbang/%d_ins_%d.bin", szProcName, size_int_, (int)gettidv1());
  ...
  extern "C" void xiaojianbangFunctionInvoke(ArtMethod *artMethod) REQUIRES_SHARED(Locks::mutator_lock_)
  {
    Thread *self = nullptr;
    uint32_t temp = 6;
    uint32_t *args = &temp;
    ...
    artMethod->Invoke(self, args, args_size, result, "xiaojianbang");
  }
  // add
  ...
  ArtMethod *ArtMethod::FindOverriddenMethod(PointerSize pointer_size)
  {
    ...
    DCHECK(result == nullptr ||
           GetInterfaceMethodIfProxy(pointer_size)->HasSameNameAndSignature(result->GetInterfaceMethodIfProxy(pointer_size)));
    return result;
  }
  ...
  void ArtMethod::SetIntrinsic(uint32_t intrinsic)
  {
    ...
    DCHECK(IsStatic() || IsFinal() || GetDeclaringClass()->IsFinal()) << "Potential conflict with kAccSingleImplementation";
